...
angular.module('survey').controller('surveyController', function($scope, $rootScope, $location, $anchorScroll) {
...
$scope.haircuts = $scope.shuffle($scope.questions[5]['options']);
    ...
    console.log($scope.available_options);

    $scope.update_options = function(question_number, old_value){
        for (var q in $scope.page4){
            if(question_number != $scope.page4[q].number){
                var index = $scope.available_options[$scope.page4[q].number].indexOf($scope.questions[question_number-1]['ranking']);
                $scope.available_options[$scope.page4[q].number].splice(index,1);
                if (old_value){
                    $scope.available_options[$scope.page4[q].number].push(parseInt(old_value));
                    $scope.available_options[$scope.page4[q].number].sort(function(a,b){return a - b});
                }
            }
        }
    }
    ...
    $scope.stores = $scope.shuffle($scope.stores);
  ...
  $scope.validate_capture = function(section, check_for){
  var section_questions = [];
  for (var s in section){
    section_questions.push($scope.questions[section[s].number-1]);

    if (!$scope.questions[section[s].number-1][check_for]){
        $scope.validation = false;
        return
        }
    else{
        localStorage.setItem("question_"+ s.number, $scope.questions[section[s].number-1][check_for]);
        $scope.questions[section[s].number-1]['ls_'+check_for]=localStorage.getItem("question_"+ s.number);
    }
  }
  $scope.validation = true;
  $scope.survey_page += 1;
